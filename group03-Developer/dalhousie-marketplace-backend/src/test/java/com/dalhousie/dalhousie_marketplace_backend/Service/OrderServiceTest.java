...
public class OrderServiceTest {
    ...
    private static final double PRICE_TWO = 20.0;
    ...
    void setUp() {
        ...
        mockListing.setPrice(PRICE);

        cartItem = new CartItem();
        cartItem.setListing(mockListing);
        cartItem.setQuantity(QUANTITY);
        cartItem.setPrice(BigDecimal.valueOf(PRICE_TWO));

        mockCart = new Cart();
        mockCart.setUserId(1L);
        mockCart.setCartItems(Collections.singletonList(cartItem));
        mockCart.setTotalPrice(BigDecimal.valueOf(PRICE_TWO));

        mockOrder = new Order();
        mockOrder.setOrderId(1L);
        mockOrder.setUserId(1L);
        mockOrder.setTotalPrice(BigDecimal.valueOf(PRICE_TWO));
