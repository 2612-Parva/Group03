...
public class ReviewServiceTest {
    ...
    private static final double TEST_LISTING_PRICE = 99.99;
    private static final int INITIAL_RATING = 5;
    private static final int UPDATED_RATING = 4;
    ...
    void setUp() {
        ...
        testListing.setPrice(TEST_LISTING_PRICE);
        ...
        testReview.setRating(INITIAL_RATING);
        ...
        testReviewRequest.setRating(INITIAL_RATING);
        ...
        mockReviewResponse.setRating(INITIAL_RATING);
    ...
    void testCreateReview_Success() {
        ...
        assertEquals(INITIAL_RATING, response.getRating());
    ...
    void testCreateReview_ListingMismatch() {
        // Arrange
        testOrderItem.getListing().setId(INVALID_ID); // Different from request
        when(orderItemRepository.findById(ORDER_ITEM_ID)).thenReturn(Optional.of(testOrderItem));
    ...
    void testCreateReview_UserMismatch() {
        // Arrange
        testOrder.setUserId(INVALID_ID); // Different from requesting user
        when(orderItemRepository.findById(ORDER_ITEM_ID)).thenReturn(Optional.of(testOrderItem));
    ...
    void testUpdateReview_Success() {
        // Arrange
        testReviewRequest.setRating(UPDATED_RATING);
    ...
    void testUpdateReview_NotOwnedByUser() {
        // Arrange
        testReview.setUserId(INVALID_ID); // Different user
        when(reviewRepository.findById(REVIEW_ID)).thenReturn(Optional.of(testReview));
    ...
    void testGetAverageRatingForListing() {
        // Arrange
        when(reviewRepository.getAverageRatingForListing(LISTING_ID)).thenReturn(AVERAGE_RATING);

        // Act
        Double averageRating = reviewService.getAverageRatingForListing(LISTING_ID);

        // Assert
        assertEquals(AVERAGE_RATING, averageRating);
    ...
    void testGetReviewCountForListing() {
        // Arrange
        when(reviewRepository.getReviewCountForListing(LISTING_ID)).thenReturn(REVIEW_COUNT);

        // Act
        Long count = reviewService.getReviewCountForListing(LISTING_ID);

        // Assert
        assertEquals(REVIEW_COUNT, count);
