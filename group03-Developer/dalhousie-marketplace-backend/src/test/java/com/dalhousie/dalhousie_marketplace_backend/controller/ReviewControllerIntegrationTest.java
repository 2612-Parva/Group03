...
public class ReviewControllerIntegrationTest {
    ...
    private static final int REVIEW_SIZE = 2;
    private static final long REVIEW_ID = 2L;
    private static final double AVG_RATING = 4.5;
    ...
    void testGetReviewsByListingId_Success() throws Exception {
        ...
        when(reviewService.getAverageRatingForListing(1L)).thenReturn(AVG_RATING);
        when(reviewService.getReviewCountForListing(1L)).thenReturn(REVIEW_ID);

        mockMvc.perform(get("/api/reviews/listing/{listingId}", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.reviews.size()").value(REVIEW_SIZE))
                .andExpect(jsonPath("$.averageRating").value(AVG_RATING))
    ...
    void testGetEligibleItemsForReview_Success() throws Exception {
        ...
        mockMvc.perform(get("/api/reviews/eligible-items")
                ...
                .andExpect(jsonPath("$.eligibleItems[0].price").value(PRICE));  // Adjusted path to price
    }
