...
public class UserPreferencesServiceTest {
    ...
    private static final int INITIAL_KEYWORD_COUNT = 2;
    ...
    void getUserPreferences_ReturnsNullWhenNotFound() {
        when(userPreferencesRepository.findByUser_UserId(NON_EXISTENT_USER_ID)).thenReturn(null);

        UserPreferences result = userPreferencesService.getUserPreferences(NON_EXISTENT_USER_ID);
...
//    }
    ...
    void addKeyword_DoesNotAddNullKeyword() {
        ...
        assertEquals(INITIAL_KEYWORD_COUNT, existingPreferences.getKeywords().size());
    }
